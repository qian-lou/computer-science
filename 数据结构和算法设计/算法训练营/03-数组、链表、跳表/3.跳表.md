...
![链表.png](https://i.loli.net/2021/10/19/qVeWEdZPgU4ojbM.png)
...
​	跳表，又叫做跳跃表、跳跃列表，在有序链表的基础上增加了“跳跃″的功能，由 `William Pugh`于1990年发布，设计的初衷是为了取代平衡树（比如红黑树）

**应用**：

​	`Redis`中的 `SortedSet`、 `LevelDB`中的 `MemTable`都用到了跳表，`Redis`、 `LeveDB`都是著名的`Key-Vaue`数据库
...
![ca63625ffeb04dd9e914294e180f3b0e.jpeg](https://i.loli.net/2021/10/19/E3SWDJFC5jAypGP.jpg)
...
![1.png](https://i.loli.net/2021/10/20/bzH7dEnPWCIlqfu.png)
...
![2.png](https://i.loli.net/2021/10/20/LnpckQXMbhlg5dS.png)
...
![3.png](https://i.loli.net/2021/10/20/iBAeXmsWzh9pfVc.png)
...
![4.png](https://i.loli.net/2021/10/20/tONxqiYsb25WZgF.png)
...
![5.png](https://i.loli.net/2021/10/20/34JH8M56yaGcEfZ.png)
...
![6.png](https://i.loli.net/2021/10/20/MhbBDx8RVArPywC.png)
...
![7.png](https://i.loli.net/2021/10/20/kEUJXIfjCgn2pi9.png)
...
![8.png](https://i.loli.net/2021/10/20/aQfvLDUTp9HhEG6.png)
...
![9.png](https://i.loli.net/2021/10/20/gdOEkVB5QzPrJHp.png)
...
![10.png](https://i.loli.net/2021/10/20/guzEtHFTZcb2wG6.png)
...
![11.png](https://i.loli.net/2021/10/20/ok3DFh5jTLBRq8O.png)
...
![15.png](https://i.loli.net/2021/10/20/FW4JKl1xM9iDkNj.png)
...
![16.png](https://i.loli.net/2021/10/20/mVLjcf8GESCMdWi.png)
...
在博客上从来没有见过有讲述 `HBase MemStore` 的数据结构，其实 `HBase MemStore` 内部存储数据就使用的跳表。为什么呢？`HBase` 属于 `LSM Tree` 结构的数据库，`LSM Tree` 结构的数据库有个特点，实时写入的数据先写入到内存，内存达到阈值往磁盘 flush 的时候，会生成类似于 `StoreFile` 的**有序文件**，而跳表恰好就是天然有序的，所以在 flush 的时候效率很高，而且跳表查找、插入、删除性能都很高，这应该是 `HBase MemStore` 内部存储数据使用跳表的原因之一。`HBase` 使用的是 `java.util.concurrent` 下的 `ConcurrentSkipListMap()`。`Google` 开源的 `key/value` 存储引擎 `LevelDB` 以及 `Facebook` 基于 `LevelDB` 优化的 `RocksDB` 都是 `LSM Tree` 结构的数据库，他们内部的 `MemTable` 都是使用了跳表这种数据结构。
...
public class SkipList<K, V> {
    ...
    public V remove(K key) {
        ...
        int removeLevel = removeNode.nexts.length;
...
### 参考：[https://www.jianshu.com/p/9d8296562806](https://www.jianshu.com/p/9d8296562806)
